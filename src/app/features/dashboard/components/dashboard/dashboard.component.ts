...
export class DashboardComponent implements OnInit {
  ...
  loadData() {
    this.users = this.userService.getUsersWithTaskStats();
  ...
  // CODE SMELL: Complex sorting logic in component
  ...
  sortUsers(column: string) {
    ...
    // CODE SMELL: Mutation of array, imperative sorting
  ...
  // CODE SMELL: Called for every user in template - performance issue
  // VIOLATION: Change Detection - causes multiple recalculations
  getPerformanceScore(userId: number): number {
    return this.userService.calculateUserPerformanceScore(userId);
  }

  // CODE SMELL: Another method called in template loop
  // PERFORMANCE ISSUE: Nested loop execution on every change detection
  ...
  // CODE SMELL: Side effects, multiple responsibilities
  showUserDetails(userId: number) {
    // CODE SMELL: Manual loop instead of find
    for (let i = 0; i < this.users.length; i++) {
    ...
    // CODE SMELL: Filtering logic that could be in service
    this.userTasks = [];
    for (let i = 0; i < this.tasks.length; i++) {
      if (this.tasks[i].userId === userId) {
  ...
  // CODE SMELL: Formatting logic in component
  // VIOLATION: Single Responsibility
